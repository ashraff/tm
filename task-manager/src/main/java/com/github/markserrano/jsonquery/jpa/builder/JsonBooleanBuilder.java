	...
	public BooleanBuilder build(JsonFilter jsonFilter) {
		...
		for (JsonFilter.Rule rule : filter.getRules()) {
			if (rule.getJunction() == null)
				rule.setJunction(filter.getGroupOp().toLowerCase());
			builder = build(builder, rule);
		}
		return builder;
	}

	public BooleanBuilder buildRecusively(JsonFilter jsonFilter,
		...
		for (JsonFilter.Rule rule : jsonFilter.getRules()) {
			if (rule.getJunction() == null)
				rule.setJunction(jsonFilter.getGroupOp().toLowerCase());
			builder = build(builder, rule);
		}

		if (jsonFilter.getGroups() != null && !jsonFilter.getGroups().isEmpty()) {
			Iterator<JsonFilter> i = jsonFilter.getGroups().iterator();
			while (i.hasNext()) {
				JsonFilter filter = i.next();
				if(jsonFilter.getGroupOp().equals("OR"))
					builder = builder.or(buildRecusively(filter,
							null).getValue());
				else builder = builder.and(buildRecusively(filter,
						null).getValue());
				i.remove();
			}
		}
